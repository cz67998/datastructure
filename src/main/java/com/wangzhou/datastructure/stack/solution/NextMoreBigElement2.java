 ...
 * Time:10:14
 **/
public class NextMoreBigElement2 {
    public int[] nextGreaterElement(int[] nums1, int[] nums2) {
       //取出nums2中所有存在右边的值大于当前值的数，以当前值为key，大于该值的右值为value，
        // 不存在以-1位value  放入hashmap
        Stack<Integer> stack =new Stack<>();
        HashMap<Integer,Integer> hashMap=new HashMap<>();
        //nums2为大值
        for (int i = 0; i < nums2.length; i++) {
            int st=nums2[i];
            while(! stack.isEmpty()&&stack.peek()<st){
                hashMap.put(stack.pop(),st );
            }
            stack.push(st);
        }
        while(!stack.isEmpty()){
    ...
    public boolean isExist(int a,int i,int j,int[] nums1,int[] nums2,  Stack output){
